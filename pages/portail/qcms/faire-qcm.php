<?php
    ...
    if (!isset($_GET['id'])) {
        redirect(get_full_url("pages/portail/qcms/qcms.php"));
    }
...
<div class="container-fluid p-4 mb-5">
    <div class="row p-4">
        <div class="col-12">
            <h1 class="text-custom-dark tracking-light fs-2 fw-bold leading-tight min-w-72"><?php echo htmlspecialchars($qcm['titre']); ?></h1>
            <p class="text-muted"><?php echo htmlspecialchars($qcm['description']); ?></p>
        </div>
    </div>

    <form action="<?php echo get_full_url('includes/api/resultat.php?submit'); ?>" method="post">
        <input type="hidden" name="qcm_id" value="<?php echo $qcm_id; ?>">

        <?php if ($questions): ?>
            <?php foreach ($questions as $index => $question): ?>
                <div class="card bg-theme shadow-sm mb-4">
                    <div class="card-body">
